...
public class ProductController {
    ...
    public Msg deleteBatch(Integer[] productid) {
        for (Integer i:productid){
            System.out.println("deleteBatch==id==================>"+i);
        }
        try {
    ...
    public Msg doAdd(HttpServletRequest request){
        ...
        String img = ImageUtil.upload(request,mFile);
        if (img!=null){
            System.out.println("ProductController doadd--->"+img);
            Product product =new Product();
    ...
    public String refresh(@RequestParam(value = "pageNum",defaultValue = "1")String pageNum, String category,HttpServletRequest request, HttpServletResponse response) {
        response.setContentType("text/html;charset=UTF-8");
        PageInfo pageInfo = productService.showPage(pageNum,category);
